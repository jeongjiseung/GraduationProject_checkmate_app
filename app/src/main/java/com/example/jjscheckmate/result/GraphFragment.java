...
//                            if (checked_gridresults.get(p).get(q) != 0) {
...
//                    Log.d("mawang", "GraphFragment onCreateView 라그 - xasis_name = " + xasis_name);
                    ...
                    xAxis.setValueFormatter(new ValueFormatter() {
                        ...
                        public String getFormattedValue(float value) {
                            crazy++;
                            int index = (int) value;
//                            Log.d("mawang", "GraphFragment onCreateView getFormattedValue 라그 -xAxis  float value/index/crazy = " + value+"/"+index+"/"+crazy );
...
//                    for (int p = 0; p < grid_rows.get(i).size(); p++) {
                    for (int p = 0; p < grid_cols.get(i).size(); p++) {
//                        barDatasets.add(new BarDataSet(barentries.get(p), grid_rows.get(i).get(p)));
...
//                    Log.d("mawang", "GraphFragment onCreateView 그리드 - barDatasets = " + barDatasets);
                    ...
                    barData.setValueFormatter(new ValueFormatter() {
                        int crazy; // 데이터 개수만큼 호출되네

                        @Override
                        public String getFormattedValue(float value) { // value는 해당 데이터 표시값
                            crazy++;
                            int index = (int) value;
//                            Log.d("mawang", "GraphFragment onCreateView getFormattedValue 라그 -BD value/index/crazy = " + value + "/" + index + "/" + crazy);
                            return String.valueOf(index);
                        }
                    });

                    barChart.setData(barData);
//                    Log.d("mawang", "GraphFragment onCreateView 라그 - barData = " + barData.getDataSetByIndex(0)); // work
