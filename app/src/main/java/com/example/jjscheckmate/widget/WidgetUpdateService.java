...
import static com.example.jjscheckmate.widget.HomeSurveyWidget.ACTION_REFRESH_PRESSED;
...
public class WidgetUpdateService extends Service {
    ...
    // onCreate - onStartCommand - buildUpdate 순서
    @Override
...
//        }
        ...
        Log.d("mawang","WidgetUpdateService onCreate");
        preferences = getSharedPreferences("jsPref", Context.MODE_PRIVATE);
    }

    @Override
    public int onStartCommand(Intent intent, int flags, int startId){
        Log.d("mawang","WidgetUpdateService onStartCommand");
    ...
    private RemoteViews buildUpdate(Context context, int appWidgetId){
        Log.d("mawang","WidgetUpdateService buildUpdate");
...
//        editor.apply();
    ...
    private void setRefreshView(Context context, RemoteViews remoteViews){
        Log.d("mawang","WidgetUpdateService setRefreshView");

        if(isRefreshPressed){
            Log.d("mawang","WidgetUpdateService setRefreshView - inside");
//            if(preferences.getInt("_id", -1) == -1){return;}
...
//            setRefreshViewThread.start();
    ...
    private void setWidgetView(Context context, RemoteViews remoteViews){
        ...
        Log.d("mawang","WidgetUpdateService setWidgetView - isRefreshPressed :"+Boolean.toString(isRefreshPressed));
...
//        }
        ...
        RetrofitApi.getService().get_idSurveyList(id).enqueue(new retrofit2.Callback<ArrayList<UploadedSurveyDTO>>() {
            ...
            public void onResponse(retrofit2.Call<ArrayList<UploadedSurveyDTO>> call, retrofit2.Response<ArrayList<UploadedSurveyDTO>> response) {
                ...
                if(response.isSuccessful()) {
                    ...
                    Log.d("mawang","WidgetUpdateService get_idSurveyList - response.body() :"+response.body().toString());
                }
            }
            @Override
            public void onFailure(retrofit2.Call<ArrayList<UploadedSurveyDTO>> call, Throwable t) {
                Log.d("mawang","WidgetUpdateService get_idSurveyList - onFailure :"+t.getMessage());
            }
        });
    ...
    private void setPressRefreshButton(Context context, RemoteViews remoteViews, int appwidgetId){
        Log.d("mawang","WidgetUpdateService setPressRefreshButton");
...
//        remoteViews.setOnClickPendingIntent(R.id.homewidget_refresh, pendingIntent);
    ...
    private void setPressSettingSurvey(Context context, RemoteViews remoteViews, int appwidgetId){
        Log.d("mawang","WidgetUpdateService setPressSettingSurvey");
