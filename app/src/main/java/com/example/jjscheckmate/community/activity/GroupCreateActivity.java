...
//                    Toast.makeText(GroupCreateActivity.this,"btnCategory",Toast.LENGTH_SHORT).show();
...
//                        finish(); // 여기서 끝내면 GroupFragment findAllGroup 과 동시에 진행되서 서버 렉 걸림
                        Log.d("mawang","GroupCreateActivity ClickListener btnGroupCreate - 그룹생성!!!");
                    }
                    break;
                }
            }
    ...
    public HashMap<String,Object> dataParse(){
        ...
        Log.d("mawang","GroupCreateActivity dataParse - map: "+map);
    ...
    public void groupCreate(HashMap<String,Object> map){
        ...
        RetrofitApi.getService().groupCreate(map,part).enqueue(new Callback<Boolean>() {
            ...
            public void onResponse(Call<Boolean> call, Response<Boolean> response) {
                Log.d("mawang","GroupCreateActivity groupCreate - 성공");
                finish(); // 생성 후에 GroupFragment 가 findAllGroup 할 수 있게 하자.
            }
            @Override
            public void onFailure(Call<Boolean> call, Throwable t) {
                Log.d("mawang","GroupCreateActivity groupCreate - 실패");
            }
        });
    }
    ...
    boolean inputCheck(){
        ...
        if (btnPrivate && groupPassword.getText().toString().equals("")) {
            Toast.makeText(this,"비공개 일 경우 비밀번호필수",Toast.LENGTH_SHORT).show();
            return false;
        }
